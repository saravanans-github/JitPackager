...
@implementation NSData (FastHexV2)
...
+ (instancetype)dataWithHexCString:(const char *)hexString
{ return [[self alloc] initWithHexCString:hexString ignoreOtherCharacters:YES]; }
...
- (nullable instancetype)initWithHexCString:(const char*)hexString ignoreOtherCharacters:(BOOL)ignoreOtherCharacters
{
    if (!hexString)
        return nil;
    
    const NSUInteger charLength = strlen(hexString);
    const NSUInteger maxByteLength = charLength/2;
    uint8_t *const bytes = malloc(maxByteLength);
    uint8_t *bytePtr = bytes;
    ...
    for (CFIndex i = 0; i < charLength; ++i) {
        ...
        uint8_t nextNibble = nibbleFromChar(hexString[i]);
